...
function bundleBinaryAssets() {
    ...
    for (let file of glob.sync("./node_modules/font-awesome/fonts/*.*")) {
        ...
        fs.copyFileSync(
            file,
            path.join("./public/fonts/", path.basename(file))
        );
    }
    if (process.argv.includes("--gzip")) {
...
function bundleWebAppFiles() {
    ...
    Promise.all(
        webapp_icons.map((icon) => {
            return Jimp.read("./img/app.png").then((file) => {
                file.resize(icon.size, Jimp.AUTO, Jimp.RESIZE_BEZIER).write(
                    path.join("./public/img/", icon.name)
                );
            });
        })
    ).then(() => {
    ...
    Promise.all(
        webapp_splash_screens.map((dim) => {
            return Jimp.read("./img/splash.png").then((file) => {
...
function watch() {
    ...
    b.plugin(watchify, { poll: polling });
